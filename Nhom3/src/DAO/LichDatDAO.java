...
public class LichDatDAO implements HairSalonDAO<LichDat, Integer>{
    ...
    String update = "update lichdat set ngaybatdau=?,ngayketthuc=?,ghichu=?,makhachhang=?,manhanvien=? where madatlich=?";
    ...
    public void insert(LichDat model) {
        try {
            JdbcHelper.executeUpdate(insert, model.getNgayBatDau(),model.getNgayKeyThuc(),model.getGhiChu(),
                    model.getMaKhachHang(),model.getMaNhanVien());
    ...
    public void update(LichDat model) {
        try {
            JdbcHelper.executeUpdate(update, model.getNgayBatDau(),model.getNgayKeyThuc(),model.getGhiChu(),
                    model.getMaKhachHang(),model.getMaNhanVien(),model.getMaLichDat());
        } catch (SQLException ex) {
            Logger.getLogger(LichDatDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
